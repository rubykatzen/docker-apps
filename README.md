...
Variables use a two-tier cascade — each level overrides the previous:
...
Auto-generated by `up.sh` — do not edit. Contains all server vars plus `APP_NAME` set to the app folder name.
...
Set `MYAPP_SOME_PATH` in the server's `.env` to override for that app only. See `AGENTS.md` for the full naming convention.
