...
steps:
  - uses: actions/checkout@v7
    with:
      ref: v1.3.1
  - uses: rubykatzen/flightdeck/.github/actions/build-bundle@v1.3.1
    with:
      paths: apps
...
steps:
  - uses: actions/checkout@v7
    with:
      ref: v1.3.1
  - uses: rubykatzen/flightdeck/.github/actions/build-apps-bundle@v1.3.1
    with:
      release-tag: v1.3.1
      token: ${{ secrets.GITHUB_TOKEN }}
```
...
jobs:
  deploy:
    ...
    steps:
      - uses: actions/checkout@v7
      - uses: rubykatzen/flightdeck/.github/actions/deploy@v1.3.1
        id: deploy
        with:
      ...
      - name: Notify Telegram
        uses: rubykatzen/baseline/.github/actions/send-telegram-message@v1.3.1
        with:
...
The `@v1.3.1` pin on the `uses:` line only controls which ref this action's own code runs at. `target-manifest`'s own `app_refs` entries are separate and don't have to match it. <!-- x-release-please-version -->
